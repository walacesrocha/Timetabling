...
A estrutura de vizinhança usada neste trabalho foi primeiro usada por \cite{CDGS11b}, mas para o \textit{track} 2 do ITC-2007. 
...
Pode se optar por construir os dois caminhos, com a desvantagem que o tempo de execução é o dobro. \cite{Resende05graspwith} cita que no caso de fazer a opção por somente uma das trajetórias, as melhores soluções geralmente são encontradas utilizandoo religamento inverso. A explicação para isso é que boas soluções tendem a estar próximas à solução mais promissora. Sendo assim, o GRASP implementado aplica \textit{Path-relinking} usando uma solução elite como a inicial e a solução obtida na busca local é a alvo.
